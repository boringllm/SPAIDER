...
def _model_params(session) -> list[dict]:
    """The per-role model configuration this session runs under (for the report), taken from the
    session's config. API keys are stripped and internal (underscore-prefixed) fields dropped."""
    models = (getattr(session, "cfg", None) or {}).get("models") or {}
    out: list[dict] = []
    for role, mc in models.items():
