...
function buildDashboardHTML(d) {
  ...
  // Model parameters each role ran under (config snapshot; api_key already stripped server-side).
  const MP_COLS = ["provider", "model", "max_tokens", "max_turns", "temperature", "top_p", "thinking", "thinking_budget", "effort"];
  const mpHead = `<tr><th>role</th>${MP_COLS.map(c => `<th>${esc(c)}</th>`).join("")}</tr>`;
  const cell = v => esc(v == null || v === "" ? "—" : v);
  const mpRows = (d.models || []).map(m => `<tr><td><b>${esc(m.role)}</b></td>${MP_COLS.map(c => `<td>${cell(m[c])}</td>`).join("")}</tr>`).join("")
    || `<tr><td colspan="${MP_COLS.length + 1}">—</td></tr>`;
  const css = `
...
<body><div class="page">
  ...
  <div class="tbl"><h2>Model parameters</h2><table>${mpHead}${mpRows}</table></div>
