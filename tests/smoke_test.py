...
def test_rbac_and_targets() -> None:
    ...
    # edit_session only for the OWNER (with the cap), never a granted viewer — but an ADMIN has full
    # control over ANY user's session (rename/delete/edit), not just their own.
    ...
    check("admin can edit/rename ANY user's session", _can(admin, "edit_session", sess_alice, cfg))
...
async def test_session_rename() -> None:
    ...
    # admin ownership transfer (set_owner) — reassigns and persists
    await sess.set_owner("u_new_admin")
    check("owner reassigned", sess.owner == "u_new_admin")
    rows = await db.list_sessions(owner=None)
    check("owner change persisted to db", any(r["id"] == sid and r["owner"] == "u_new_admin" for r in rows))
    try:
        await sess.set_owner("  ")
        check("empty owner rejected", False)
    except ValueError:
        check("empty owner rejected", True)
    await sess.shutdown()
...
def test_dashboard_compute() -> None:
    ...
    session = SimpleNamespace(id="s1", name="demo", target="10.0.0.1", status="done",
                              ...
                              cfg={"models": {"recon": {"provider": "anthropic", "model": "claude",
                                                        "api_key": "sk-secret", "max_tokens": 8000,
                                                        "temperature": 0.4, "_client_proxy": None}}})
    ...
    check("model parameters included in the report", any(m.get("role") == "recon" for m in d["models"]))
    _recon_mp = next(m for m in d["models"] if m["role"] == "recon")
    check("model params carry tunables", _recon_mp.get("model") == "claude" and _recon_mp.get("max_tokens") == 8000)
    check("model params strip api_key + internal fields", "api_key" not in _recon_mp and "_client_proxy" not in _recon_mp)
...
async def test_concise_nudge_on_max_tokens() -> None:
    """A sub-agent whose OUTPUT is truncated at max_tokens is answered with a 'be more concise'
    message and retries, instead of proceeding on a cut-off turn. The orchestrator is exempt."""
    import tempfile

    from spider import config
    from spider.db import Database
    from spider.llm import LLMResponse, Usage
    from spider.session import Session

    tmp = Path(tempfile.mkdtemp(prefix="spider_concise_"))
    cfg = _mock_cfg()
    cfg["workspace_root"] = str(tmp / "workspaces")
    cfg["agents_dir"] = str(tmp / "agents")
    config.CONFIG_DIR = tmp / "config"
    sess = Session("concise", "t", cfg, Database(str(tmp / "c.db")))
    await sess.setup()
    parent = await sess.create_agent("orchestrator", "lead", parent=None)
    child = await sess.create_agent("recon", "recon the target", parent=parent)
    ...
    class FakeProvider:
        ...
        async def complete(self, system, messages, tools, on_token=None):
            self.calls += 1
            if self.calls == 1:  # truncated: hit the output cap
                return LLMResponse(text="way too many words " * 4, tool_calls=[], usage=Usage(),
                                   stop_reason="max_tokens",
                                   raw_content=[{"type": "text", "text": "way too many words"}])
            # concise retry: finish cleanly
            return LLMResponse(text="done", usage=Usage(), stop_reason="tool_use",
                               tool_calls=[{"id": "t1", "name": "finish", "input": {"summary": "ok"}}],
                               raw_content=[{"type": "text", "text": "done"},
                                            {"type": "tool_use", "id": "t1", "name": "finish", "input": {"summary": "ok"}}])

    fake = FakeProvider()
    child._provider = fake
    await asyncio.wait_for(child._loop(), timeout=20)
        ...
        for m in agent.messages:
            ...
            for c in (m.get("content") or []):
                if isinstance(c, dict):
                    out.append(c.get("content") if c.get("type") == "tool_result" else c.get("text"))
        return [t for t in out if t]

    nudged = any("concise" in (t or "").lower() for t in _user_texts(child))
    check("max_tokens truncation answered with a be-concise message", nudged)
...
def test_security_fixes() -> None:
    ...
    # ownership transfer is an admin-only action
    check("/api/sessions/{sid}/owner is admin-gated", "require_admin" in _route_deps("/api/sessions/{sid}/owner", "POST"))
...
def main() -> int:
    ...
    print("- concise nudge on max_tokens"); asyncio.run(test_concise_nudge_on_max_tokens())
